...
int NightTime = 600; /*Time ESP will go to sleep 10 hours (10*60min)*/
...
void setup() {time_t now;
  ...
  if ((currentHour >= 21) || (currentHour < 6)){
    esp_sleep_enable_timer_wakeup(NightTime* 60 * uS_TO_S_FACTOR);
    Serial.println("Good Night and see you tomorrow");
  }
  else {
...
void TakePhoto() {
  ...
  if ((takeNewPhoto)&&(currentMinute%2==1)) {
    takeNewPhoto = false; 
    ...
    takeNewPhoto = true;
    Serial.println("Genap..., tidak ambil photo dulu");
  }
  printLocalTime();  
  delay(3000);
}
